...
export default function UseRef03() {
  ...
  // ? id값을 전달 받아 삭제하고자 하는 요소를 filtering하는함수
  ...
  return (
    <div>
      {/* <Item item={initialItems[0]}></Item>
      <Item item={initialItems[1]}></Item>
      <Item item={initialItems[2]}></Item> */}
      {/* {initialItems.map((item, index) => (
        // map 과 filter 사용 시 생성되는 컴포넌트 또는 요소에는 각각을 구분할 수 있는 key값을 전달!
        <Item item={item} key={index} />
      ))} */}
      {initialItems.map(item => (
        <Item item={item} key={item.amount}></Item>
      ))}
      ...
      <>
        {items.map(item => (
          <div key={item.id}>
            <ItemComponent item={item} onRemove={handleRemove} onUpdate={handleUpdateAmount}></ItemComponent>
          </div>
        ))}
      </>
