      ...
      */}
      {users ? users.map( user => (
        <div key={user.id}>
          <h4>{user.name}</h4>
          <h4>{user.email}</h4>
          <button type='button' onClick={() => handleEdit(user)}>수정</button>
          <button type='button' onClick={() => deleteUser(user.id)}>삭제</button>
        </div>
      ...
      <form onSubmit={handleSubmit}>
        <input type="text" placeholder='id' name='id' onChange={handleInputChange} value={newUser.id}/>
        <input type="text" placeholder='name' name='name' onChange={handleInputChange} value={newUser.name}/>
        <input type="text" placeholder='email' name='email' onChange={handleInputChange}
        value={newUser.email}/>
        {/* <button type='submit'>사용자 추가</button> */}
        {/* <button type='submit'>사용자 수정</button> */}
        <button type='submit'> {editingUserId ? '사용자 수정' : '사용자 추가'}</button>
      </form>
    </div>
  )
}
