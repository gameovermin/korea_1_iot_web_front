...
type Student = '이승아' | '이도경' | '정재원' | '최준혁' | '최소윤';
...
// ! 객체의 구조적 타이핑(덕 타이핑)
// : 객체의 타입을 실제 값보다는 그 구조나 멤버에 의해결정하는 방식
...
userA.address.city = '도르트문트';
userA.address.street = '12345 3t';
console.log(userA);
/*
{
  username: '마르코로이스',
  email: 'qwer1234@ccc.com',
  address: { street: '12345 3t', city: '도르트문트', zipLode: '몰라' }  
}
*/
...
type UserDataType = {
  name: string; // - 일반적인 객체 소것ㅇ 타입 명시

  // ? 인덱스 서명 사용 방법
  // [propertyName: indexType]: valueType;
  // 키의 타입과 값의 타입을 미리 명시
  [key: string]: string | number | boolean;  // 키는 string 사용 권장
