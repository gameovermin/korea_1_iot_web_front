...
// # 3. 제네릭 기본 문법
// : 사용할 컴포넌트(변수, 함수, 클래스 등)의 이름 뒤에 꺽새괄호(<>) 안에 "타입 변수"를 지정
...
let str1 = generic<string>('하이요'); //  함수 호출 시 타입 변수에 활용할 타입을 명시
...
let pairOutput1 = pair<string, number>('ㅎㅇ', 132);
let pairOutput2 = pair<number, string>(132, 'ㅎㅇ');
...
let myGenericNumber2 = new GenericClass<string>('', function (x, y) {return `${x} + ${y}`});

console.log(myGenericNumber2.add('크', '산테'));