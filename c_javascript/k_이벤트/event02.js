...
function bgChange(event) {
  const randomColor = backgroundChange();
...
// ! 2. 이벤트의 기본행동 방지
// form 태그의 경우 submit 버튼 실행 시 내부의 데이터가 서버에 전송되면서 페이지가 리로드됨

// 기본 행동을 방지하는 메서드 : preventDefault()
...
childBtn.addEventListener('click', function(e) {
  ...
  // : 이벤트 객체.stopPropagation() 메서드를 사용하여 전파를 중지
