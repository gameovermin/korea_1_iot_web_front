...
userData.nickname = userData.nickname !== undefined ? userData.nickname : '익명';  

// 2. 객체에서 name과 nickname을 각각 변수 userName과 userNickname에 할당하세요. nickname이 없다면, '익명'으로 할당

// let userName = userData.name;
// let userNickname = userData.nickname !== undefined ? userData.nickname : '익명';  

const {name: name, nickname: nickname = '익명값으로 수정'} = userData;
...
// let updateedTeamMembers = teamMembers.map(data => data).push('장서연').push('최현우');

let updateedTeamMembers = [...teamMembers, '장서연', '최현우'];

console.log(updateedTeamMembers);
